(function () {
  ...
  // Собираем контент — 6 полных секций
  let html = "";
  ...
  if (parsed.totem) {
    ...
    if (parsed.totem.includes("ягуар")) {
      html +=
        "<p>Подобно ягуару, ты видишь в темноте то, что скрыто от других. Твоя интуиция — это твой компас. Ты прирожденный лидер, но предпочитаешь действовать из тени, нанося удар только тогда, когда уверен в победе.</p>";
    } else if (parsed.totem.includes("змея")) {
      html +=
        "<p>Подобно змее, ты проходишь через множество трансформаций, сбрасывая старую кожу убеждений и страхов. Ты — целитель, чья мудрость приходит через глубокое понимание циклов жизни и смерти.</p>";
    } else if (parsed.totem.includes("ветер") || parsed.totem.includes("ик")) {
      html +=
        "<p>Ты — неукротимый ветер перемен. Твоя сила в слове и общении. Ты несешь новые идеи и вдохновение, но помни: даже легкий бриз способен создать бурю. Направляй свою энергию осознанно.</p>";
    } else {
      html +=
        "<p>Твой дух-хранитель — древняя сила, источник твоей внутренней мощи. Ты обладаешь уникальной способностью влиять на мир вокруг себя, и в 2026 году эта сила раскроется в полной мере.</p>";
    }
  }
  html += "</div></div>";
  ...
  if (parsed.stone) {
    ...
    html +=
      "<p>Носи его ближе к телу в важные дни, медитируй с ним, и он впитает в себя все тревоги, наполняя тебя спокойствием и уверенностью. Этот камень — твой якорь в мире перемен.</p>";
  }
  html += "</div></div>";
  ...
  if (parsed.color) {
    ...
    html +=
      "<p>Окружи себя этим цветом: добавь его в одежду, аксессуары или интерьер. Он будет работать как маяк, притягивая к тебе нужные события и отталкивая нежелательные влияния. В дни, когда нужна особая поддержка, этот цвет — твоя броня.</p>";
  }
  html += "</div></div>";
  ...
  if (parsed.powerMonth) {
    ...
    html +=
      "<p>В этот период твоя энергия будет на пике. Запускай проекты, назначай важные встречи, делай предложения. Сама Вселенная будет открывать перед тобой двери. Не упусти это время — оно особенное.</p>";
  }
  html += "</div></div>";
  ...
  if (parsed.dangerMonth) {
    ...
    html +=
      "<p>В этот период не принимай поспешных решений и не ввязывайся в авантюры. Посвяти время планированию, отдыху и восстановлению. Конфликты и финансовые риски могут иметь долгие последствия. Будь особенно внимателен к знакам вокруг.</p>";
  }
  html += "</div></div>";
  ...
  if (parsed.advice) {
    ...
    html +=
      "<p>Одна фраза, которая изменит твой 2026. Пусть эта мудрость будет твоим путеводным светом. Это не просто слова — это ключ к твоей реализации и счастью в наступающем году.</p>";
  }
  html += "</div></div>";
